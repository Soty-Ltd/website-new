...
<svelte:head>
	...
	<meta property="twitter:domain" content="soty.one" />
	<meta property="twitter:url" content="https://www.soty.one{$page.url.pathname.toString()}" />
   ...
   "@type": "Website",
   "name": "${title} | soty.one,
   "url": "https//www.soty.one${$page.url.pathname}",
   "logo": ${image}  }</script>`}
</svelte:head>
